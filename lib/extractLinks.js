function extractLinks(markdownContent, filePath) {
  ...
  const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
  const matches = []; // Se declara una variable matches como un arreglo para almacenar los enlaces encontrados
  ...
  while ((match = linkRegex.exec(markdownContent)) !== null) {
    // Extraemos el texto y la URL del enlace del objeto match
    const text = match[1];
    const url = match[2];
  ...
  // Finalmente, retornamos el arreglo "matches" que contiene todos los enlaces encontrados.
  return matches;
}
